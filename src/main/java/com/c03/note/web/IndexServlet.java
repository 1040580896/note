...
public class IndexServlet extends HttpServlet {
    ...
    private void noteList(HttpServletRequest request, HttpServletResponse response,
        ...
        // 3. 调用Service层查询方法，返回Page对象
        Page<Note> page = new NoteServiceImpl().findNoteListByPage(pageNum, pageSize, user.getUserId(), title, date, typeId);

        // 4. 将page对象设置到request作用域中
        request.setAttribute("page", page);

        // 通过日期分组查询当前登录用户下的云记数量
        List<NoteVo> dateInfo = new NoteServiceImpl().findNoteCountByDate(user.getUserId());
        // 设置集合存放在request作用域中
        request.getSession().setAttribute("dateInfo", dateInfo);

        // 通过类型分组查询当前登录用户下的云记数量
        List<NoteVo> typeInfo = new NoteServiceImpl().findNoteCountByType(user.getUserId());
        // 设置集合存放在request作用域中
